...
- (void)setNeedsNavigationBackground:(CGFloat)alpha {
    if (self.navigationBar.subviews.count > 0){
        UIView *barImageView = self.navigationBar.subviews.firstObject;
...
- (void)et__updateInteractiveTransition:(CGFloat)percentComplete {
    ...
    if (topVC != nil) {
        ...
        if (coor != nil) {
            CGFloat fromAlpha = [(BaseUIViewController *)[coor viewControllerForKey:UITransitionContextFromViewControllerKey] getCurrentAlpha];
            CGFloat toAlpha = [(BaseUIViewController *)[coor viewControllerForKey:UITransitionContextToViewControllerKey] getCurrentAlpha];
...
//#pragma clang diagnostic ignored "-Wdeprecated-declarations" (忽略方法弃用警告)
...
- (void)dealInteractionChanges:(id<UIViewControllerTransitionCoordinatorContext>)context {
    if ([context isCancelled]) {// 自动取消了返回手势
        ...
        [UIView animateWithDuration:cancelDuration animations:^{
            CGFloat nowAlpha = [(BaseUIViewController *)[context viewControllerForKey:UITransitionContextFromViewControllerKey] getCurrentAlpha];
        ...
        [UIView animateWithDuration:finishDuration animations:^{
            CGFloat nowAlpha = [(BaseUIViewController *)[context viewControllerForKey:UITransitionContextToViewControllerKey] getCurrentAlpha];
